  ...
  return (
    <>
      <header className={`premium-nav ${scrolled ? 'scrolled' : ''} ${home ? 'on-home' : 'on-detail'}`}>
        <div className="nav-inner">
          ...
          >
            ...
            <span><b>{siteConfig.companyName}</b><small>Aerial Firefighting Portfolio</small></span>
          </button>
      ...
      <motion.div className={`mobile-nav-panel ${open ? 'open' : ''}`} initial={false} animate={{ opacity: open ? 1 : 0, pointerEvents: open ? 'auto' : 'none' }}>
        ...
        <div className="mobile-nav-header"><span>ASPIREX LLC</span><small>Aerial Firefighting Intelligence Portfolio</small></div>
        ...
        <div className="mobile-nav-foot"><span>3 interactive product systems</span><span>Synthetic aviation data</span></div>
      </motion.div>
    </>
  )
}
