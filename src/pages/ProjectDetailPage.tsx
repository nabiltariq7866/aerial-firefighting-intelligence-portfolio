...
function DetailArchitecture({ project }: { project: Project }) {
  const layers = [
    ['OPERATIONAL SOURCES', project.sourceData.join(' · ')],
    ['DATA / WORKFLOW LAYER', 'Structured context · State · History · Audit'],
    ['INTELLIGENCE SUPPORT', project.aiCapabilities.slice(0, 3).join(' · ')],
    ['OPERATIONAL WORKFLOW', project.workflow.slice(1, 4).join(' · ')],
    ['HUMAN AUTHORITY', 'Review · Modify · Approve · Act'],
  ]
    ...
    <div className="site-shell project-detail-shell" style={{ '--accent': project.accent, '--accent-2': project.accent2, '--dark': project.dark } as React.CSSProperties}>
      ...
      <main>
        <section className="project-detail-hero">
          ...
          <div className="detail-hero-meta"><span>SYNTHETIC DATA</span><span>HUMAN AUTHORITY</span><span>INTERACTIVE WORKFLOW</span></div>
        </section>
        ...
        <section className="detail-ai-section">
          ...
          <div className="detail-ai-inner">
            <Reveal className="detail-ai-copy"><span><ShieldCheck size={15} /> 04 · RESPONSIBLE INTELLIGENCE</span><h2>Intelligence that supports the workflow without hiding the evidence.</h2><p>{project.responsibleBoundary}</p></Reveal>
        ...
        <section className="detail-architecture-section section">
          <Reveal className="detail-section-heading"><span>05 · SYSTEM ARCHITECTURE</span><h2>Operational context → intelligence → accountable workflow.</h2></Reveal>
          <Reveal delay={.08}><DetailArchitecture project={project} /></Reveal>
          <Reveal className="detail-tech-foot">
            <p>Technology represented in this concept</p>
            <div>{project.technologies.map((technology) => <span key={technology}>{technology}</span>)}</div>
            <p>Evaluation criteria</p>
            <div>{project.successMetrics.map((metric) => <span key={metric}>{metric}</span>)}</div>
            <small>Out of demo scope: {project.outOfScope.join(' · ')}. Portfolio data and workflows are synthetic; no production operational authority is implied.</small>
          </Reveal>
        </section>
